...
// typeof - щоб перевірити будь яке значення, точніше, яке це значення за типом в JavaScript, використовуємо оператор "typeof"
...
// приведення до числа
/* якщо змінну ми обозначемо просто цифрою, котру візьмемо в лапки, то вона буде читатися як String, 
щоб привести її до числа, треба обозначити її, дати поняття Number */
...
// // Math.random() - повертає випадкове число в діапазоні (0, 1) - формула (Math.random() * (max- min) + min)
...
/* якщо до цифри додаємо цифру в лапках, то воно стає стрингою. */
...
// indexOf() - перевірити наявність літери або слова в строчці
// includes() - вичислити його індекс
...
/* утворюються за допомогою {}. обов*язково в об*єкті повинні буди значення і ключ. Ключ ЗАВЖДИ стрінга (строчка) */
...
//методи - для взаємодією з об*єктом
